"""Basically the same architecutre as my
...
all_intent = train_df['label'].append(test_df['label']).append(valid_df['label']).tolist()
...
y_train = np.array([intent_mapping[intent] for intent in train_df['label']])
y_test = np.array([intent_mapping[intent] for intent in test_df['label']])
y_valid = np.array([intent_mapping[intent] for intent in valid_df['label']])
...
# Compile the model
model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy']) # TODO Convert this to 'binary_crossentropy'.
...
intent = predict_intent(model_=model, tokenizer_=tokenizer, input_text="How are you feeling today?")
